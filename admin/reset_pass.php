...
function sendMail($admin_email, $reset_token)
{
    ...
    try {
        ...
        $mail->Body    = "We got a request from you to reset your password! <br>
            ...
            <a href='http://localhost/POS/admin/admin_password_update.php?admin_email=$admin_email&reset_token=$reset_token'>
            Reset Your Password
            </a>";
...
if (isset($_POST['send-reset-link'])) {
    $query = "SELECT * FROM `admin` WHERE `admin_email` ='$_POST[admin_email]'";
    ...
    if ($result) {
        if (mysqli_num_rows($result) == 1) {
            ...
            $date = date("Y-m-d");
            $query = "UPDATE `admin` SET `reset_token` ='$reset_token', `reset_token_expire`='$date' WHERE `admin_email`='$_POST[admin_email]'";
            if (mysqli_query($conn, $query) && sendMail($_POST['admin_email'], $reset_token)) {
             ...
             ";
                header('location:admin_password_recover.php');
            }
        }
    } else {
